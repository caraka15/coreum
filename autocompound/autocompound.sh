...
RPC_ADDRESS=$( cored status | jq -r .NodeInfo.other.rpc_address)
COIN=$( cored query staking params --node ${RPC_ADDRESS} -o j | jq -r '.bond_denom')
BASE_DENOM=$( cored query staking params --node ${RPC_ADDRESS} -o j | jq -r '.bond_denom' | sed -e 's/^.\{1\}//')
ADDRESS=$(echo $PASS |  cored keys show ${KEY_NAME} --output json | jq -r '.address')
VALOPER=$(echo $PASS |  cored keys show ${KEY_NAME} -a --bech val)
CHAIN=$( cored status --node ${RPC_ADDRESS} 2>&1 | jq -r .NodeInfo.network)
ONE_CORE=1000000
...
if [ "$ANSWER" == "yes" ]; then
    ...
    DELEGATE=$(bc <<< "$AMOUNT - $ONE_CORE" )

    if [[ $DELEGATE > 0 && $DELEGATE != "null" ]]; then
        echo "-------------------------------------------------------------------"
        echo -e "$RED$(date +%F-%H-%M-%S)$NORMAL $YELLOW To Stake = ${DELEGATE} ${COIN} $NORMAL"
        echo "-------------------------------------------------------------------"
        echo $PASS |  cored tx staking delegate ${VALOPER} ${DELEGATE}${COIN} --fees=20000utestcore --chain-id=${CHAIN} --from ${KEY_NAME} --node ${RPC_ADDRESS} -y | grep "raw_log\|txhash"
        sleep 20s
        echo "-------------------------------------------------------------------"
        echo -e "$GREEN Balance after delegation:$NORMAL"
        BAL=$( cored query bank balances ${ADDRESS} --chain-id=${CHAIN} --node ${RPC_ADDRESS} --output json | jq -r '.balances[] | select(.denom=="'${COIN}'") | .amount')
        echo -e "$YELLOW ${BAL} ${COIN} $NORMAL"
        MSG=$(echo -e " cored | $(date +%F-%H-%M-%S) | Delegated: ${DELEGATE} ${COIN} | Balance after delegation: ${BAL} ${COIN}")
    else
        MSG=$(echo -e " cored | $(date +%F-%H-%M-%S) | Insufficient balance for delegation")
        echo "-------------------------------------------------------------------"
